...
public class Deodexer {
	...
	public static boolean deoDexApkLegacy(File odexFile , File classesFile){
		String[] cmd = {"java","-jar",S.BACKSMALI_JAR,"-a",""+SessionCfg.getSdk(),"-d",S.bootTmp.getAbsolutePath(),"-x",odexFile.getAbsolutePath(),
		...
		String[] cmd2 = {"java","-jar",S.SMALI_JAR,"-a",""+SessionCfg.getSdk(),
				new File(odexFile.getParentFile().getAbsolutePath()+File.separator+"smali").getAbsolutePath() ,classesFile.getAbsolutePath()};

		// TODO search further info (can apks here have 2 classes.dex ? if so what should we do here ?)
		Process p ;
